#!/usr/bin/env python3
"""把 MaaFwApp 的 applicationId 基础包名改成 Maa_KES 使用的命名空间。

MaaFwApp 的 applicationId = BASE_APPLICATION_ID + "." + pi-profile.yaml 里的 app.id，
所以基础包名在这里改，后缀在配方文件里写（本仓库为 vero，最终 com.maafw.mq.vero）。

幂等：重复执行不会出错（已改过就直接放行）。
"""
